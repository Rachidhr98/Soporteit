...
if (-not $esAdmin -and -not $SoloLectura) {
    ...
    Write-Host ""
    Write-Host "1) Relanzar como administrador"
    ...
    if ($opcionPermisos -eq '1') {
        ...
        # [REVISADO OK] Fix del admin
        $argumentos = "-NoExit -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`""
...
# [REVISADO OK] Importar modulos (Aseguramos que todos se carguen correctamente)
...
Import-Module (Join-Path $Global:SoporteIT_ModulesPath "seguridad.psm1") -Force
